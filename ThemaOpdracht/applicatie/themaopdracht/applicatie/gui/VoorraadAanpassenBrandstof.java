...
import themaopdracht.applicatie.controller.BrandstofAanpassenControler;
...
public class VoorraadAanpassenBrandstof extends BorderPane {
	private VBox invulLijst, tekstLijst;
	private Button annuleren, bestelKnop;
	private HBox buttons;
	private TextField aantalLiterstf;
	
	public VoorraadAanpassenBrandstof(KnoppenMenu km, Stage owner, Brandstof brandstof) {
		BrandstofAanpassenControler controler = new BrandstofAanpassenControler(km);
		...
		Label aantalLiters = new Label("Nieuw aantal liters:");
		...
		annuleren.setOnAction(event -> {
			km.setDefaultScene("Wijziging afgebroken");
		});
		bestelKnop = new Button("Aanpassen");
		bestelKnop.setOnAction(event -> {
			try{
				int aantal = Integer.parseInt(aantalLiterstf.getText());
				if (aantal > 0) {
					controler.pasBrandstofAan(owner, brandstof, aantal);
				} else {
